...
router.get('/', authenticateToken, (req, res) => {
  try {
    ...
    const developer = db.prepare('SELECT user_token FROM dbt10 WHERE user_token = ?').get(userToken);
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!resolvedFarmerToken && farmerEmail) {
      const farmerStmt = db.prepare('SELECT user_token FROM dbt1 WHERE email = ?');
      ...
      resolvedFarmerToken = farmer.user_token;
    }
    ...
    const countStmt = db.prepare('SELECT COUNT(*) as count FROM dbt4 WHERE user_token = ?');
    ...
    const stmt = db.prepare(`
      INSERT INTO dbt4 (cow_name, cow_nickname, collar_id, cow_token, user_token, farm_token, state_fence, time_inside, time_outside, total_breach, timestamp)
      VALUES (?, NULL, ?, ?, ?, NULL, 'outside', 0, 0, 0, ?)
    `);
    stmt.run(cowName, macAddress, cowToken, resolvedFarmerToken, now());

    // Increment total_cow counter
    const updateStmt = db.prepare('UPDATE dbt1 SET total_cow = total_cow + 1 WHERE user_token = ?');
...
router.put('/:collarId/nickname', authenticateToken, (req, res) => {
  try {
    ...
    const isDeveloper = db.prepare('SELECT user_token FROM dbt10 WHERE user_token = ?').get(farmerToken);
...
router.delete('/:collarId', authenticateToken, (req, res) => {
  try {
    ...
    // Delete cow
    const stmt = db.prepare('DELETE FROM dbt4 WHERE collar_id = ? AND user_token = ?');
    ...
    if (result.changes > 0) {
      ...
      if (farmer && farmer.developer_token) {
        db.prepare('UPDATE dbt10 SET total_cows = total_cows - 1 WHERE user_token = ?')
      ...
      const getCowsStmt = db.prepare('SELECT id, collar_id FROM dbt4 WHERE user_token = ? ORDER BY id ASC');
...
router.get('/new', authenticateToken, (req, res) => {
  try {
    ...
    // Get all new ESP32 cows from dbt5 (these have no user_token yet)
...
router.put('/:cowToken/assign-farm', authenticateToken, (req, res) => {
  try {
    ...
    if (dbt5Cow) {
      ...
      const updateFarmerStmt = db.prepare('UPDATE dbt1 SET total_cow = total_cow + 1 WHERE user_token = ?');
      ...
      // Update dbt2 farm counters (if cow is assigned to a farm)
      if (farmToken) {
        db.prepare('UPDATE dbt2 SET total_number_of_cow = total_number_of_cow + 1 WHERE farm_token = ?')
          .run(farmToken);
        console.log(`[Assign] Incremented dbt2 total_number_of_cow for farm: ${farmToken}`);
      }
    ...
    if (isDeveloper) {
      ...
      dbt4Cow = db.prepare('SELECT * FROM dbt4 WHERE cow_token = ? AND user_token = ?').get(cowToken, farmerToken);
    }
    ...
    if (dbt4Cow) {
      ...
      // Update dbt2 farm counters
      // If cow was previously assigned to a farm, decrement that farm's count
      if (oldFarmToken) {
        db.prepare('UPDATE dbt2 SET total_number_of_cow = total_number_of_cow - 1 WHERE farm_token = ?')
          .run(oldFarmToken);
        console.log(`[Assign] Decremented dbt2 total_number_of_cow for old farm: ${oldFarmToken}`);
      }
      ...
      if (farmToken) {
        db.prepare('UPDATE dbt2 SET total_number_of_cow = total_number_of_cow + 1 WHERE farm_token = ?')
          .run(farmToken);
        console.log(`[Assign] Incremented dbt2 total_number_of_cow for new farm: ${farmToken}`);
      }

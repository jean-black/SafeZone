...
router.post('/login', async (req, res) => {
  try {
    ...
    if (isDeveloper) {
      ...
      // Update dbt10 connected user count if farmer was previously disconnected
      if (prevState === 'disconnected') {
        const developerToken = user.user_token;
        if (developerToken) {
          db.prepare('UPDATE dbt10 SET total_number_of_connected_user = total_number_of_connected_user + 1 WHERE developer_token = ?')
            .run(developerToken);
          console.log(`[Login] Incremented total_number_of_connected_user for developer: ${developerToken}`);
        }
      }
    }
...
router.post('/signup', async (req, res) => {
  try {
    ...
    const defaultDeveloper = db.prepare('SELECT user_token FROM dbt10 ORDER BY registered_at LIMIT 1').get();
    const developerToken = defaultDeveloper ? defaultDeveloper.user_token : null;

    // Insert new user (updated for new schema)
    const insertStmt = db.prepare(`
      INSERT INTO dbt1 (farmer_name, user_id, user_token, password, recovery_code, confirmation_code, total_farms, total_cows, developer_token, timestamp)
      VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)
    `);
...
router.get('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    if (userType === 'developer') {
      // Get developer data
      const stmt = db.prepare('SELECT developer_name, email, registered_at FROM dbt10 WHERE user_token = ?');
      ...
      const stmt = db.prepare('SELECT farmer_name, user_id, timestamp FROM dbt1 WHERE user_token = ?');
      user = stmt.get(userToken);
...
router.put('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    if (userType === 'developer') {
      const stmt = db.prepare('SELECT * FROM dbt10 WHERE user_token = ?');
      user = stmt.get(userToken);
      isDeveloper = true;
      if (user) {
      ...
      const stmt = db.prepare('SELECT * FROM dbt1 WHERE user_token = ?');
      user = stmt.get(userToken);
      if (user) {
    ...
    if (farmer_name && farmer_name !== currentName) {
      if (isDeveloper) {
        ...
        const checkStmt = db.prepare('SELECT farmer_name FROM dbt1 WHERE farmer_name = ? AND user_token != ?');
        const existing = checkStmt.get(farmer_name, userToken);
        ...
        // Update farmer name
        const updateStmt = db.prepare('UPDATE dbt1 SET farmer_name = ? WHERE user_token = ?');
        updateStmt.run(farmer_name, userToken);
    ...
    if (email && email !== currentEmail) {
      ...
      if (isDeveloper) {
        ...
        // Generate new confirmation code
        const confirmationCode = generateConfirmationCode();
        const updateStmt = db.prepare('UPDATE dbt1 SET user_id = ?, confirmation_code = ? WHERE user_token = ?');
        updateStmt.run(email, confirmationCode, userToken);
    ...
    if (password) {
      ...
      if (isDeveloper) {
        ...
        const updateStmt = db.prepare('UPDATE dbt1 SET password = ? WHERE user_token = ?');
        updateStmt.run(hashedPassword, userToken);
...
router.post('/logout', authenticateToken, async (req, res) => {
  try {
    ...
    if (isDeveloper) {
      ...
      const user = db.prepare('SELECT connection_state FROM dbt10 WHERE email = ?').get(userEmail);
      ...
      const user = db.prepare('SELECT connection_state, developer_token FROM dbt1 WHERE user_id = ?').get(userEmail);
      ...
      // Update dbt10 connected user count if farmer was previously connected
      if (user && user.connection_state === 'connected') {
        const developerToken = user.user_token;
        if (developerToken) {
          db.prepare('UPDATE dbt10 SET total_number_of_connected_user = total_number_of_connected_user - 1 WHERE developer_token = ?')
            .run(developerToken);
          console.log(`[Logout] Decremented total_number_of_connected_user for developer: ${developerToken}`);
        }
      }
    }

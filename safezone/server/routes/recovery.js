...
router.post('/create', authenticateToken, async (req, res) => {
  try {
    ...
    for (const cowToken of lostCowTokens) {
      const cow = db.prepare('SELECT cow_token, cow_name, user_token FROM dbt4 WHERE cow_token = ?').get(cowToken);
      if (!cow) {
    ...
    const farm = db.prepare('SELECT user_token FROM dbt2 WHERE farm_token = ?').get(farmToken);
    ...
    try {
      const farmer = db.prepare('SELECT user_id, farmer_name FROM dbt1 WHERE user_token = ?').get(farmerToken);
...
router.post('/virtual/create', authenticateToken, async (req, res) => {
  try {
    ...
    const cowNames = cows.map(c => c.cow_name); // Array of cow names for email
    try {
      const developer = db.prepare('SELECT email, developer_name FROM dbt10 WHERE user_token = ?').get(developerToken);
...
router.post('/:recoveryId/cancel', authenticateToken, async (req, res) => {
  try {
    ...
    let recovery = db.prepare('SELECT * FROM dbt7 WHERE recovery_id = ? AND user_token = ?').get(recoveryId, userToken);
    let type = 'physical';
...
router.post('/:recoveryId/share-link', authenticateToken, async (req, res) => {
  try {
    ...
    // Check if recovery exists and belongs to user
    let recovery = db.prepare('SELECT recovery_code FROM dbt7 WHERE recovery_id = ? AND user_token = ?').get(recoveryId, userToken);

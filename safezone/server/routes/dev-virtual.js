...
router.get('/farms', authenticateDeveloper, (req, res) => {
  try {
    const farms = db.prepare(`
      ...
      FROM dbt2 f
      LEFT JOIN dbt1 fr ON f.user_token = fr.user_token
      LEFT JOIN dbt10 dev ON f.user_token = dev.user_token
...
router.get('/virtual-cows', authenticateDeveloper, (req, res) => {
  try {
    const virtualCows = db.prepare(`
      ...
      LEFT JOIN dbt10 dev ON v.user_token = dev.user_token
...
router.post('/virtual-cows', authenticateDeveloper, (req, res) => {
  try {
    ...
    const updateDeveloperStmt = db.prepare('UPDATE dbt10 SET total_cows = total_cows + 1 WHERE user_token = ?');
...
router.post('/controller/select', authenticateDeveloper, (req, res) => {
  try {
    ...
    const disconnectResult = db.prepare(`
      ...
      WHERE user_token = ? AND virtual_controller_connection_state = 'connected'
    ...
    const existing = db.prepare('SELECT * FROM dbt12 WHERE user_token = ?').get(req.developerToken);
    ...
    const currentState = db.prepare('SELECT * FROM dbt12 WHERE user_token = ?').get(req.developerToken);
    if (currentState) {
      ...
      db.prepare('UPDATE dbt10 SET page19_last_used_parameter = ? WHERE user_token = ?')
...
router.post('/virtual-cows/:cowToken/position', authenticateDeveloper, (req, res) => {
  try {
    ...
    if (speed !== undefined) {
      ...
      // Verify the update
      const verification = db.prepare('SELECT last_speed_scale FROM dbt12 WHERE selected_cow_token = ? AND user_token = ?').get(cowToken, req.developerToken);
...
router.delete('/virtual-cows/:cowToken', authenticateDeveloper, (req, res) => {
  try {
    ...
    // Delete from dbt6
    const deleteStmt = db.prepare('DELETE FROM dbt6 WHERE cow_token = ? AND user_token = ?');

...
function createTestCows() {
  try {
    ...
    if (testCows.count === 0) {
      const testFarmerStmt = db.prepare('SELECT user_token FROM dbt1 LIMIT 1');
      const testFarmer = testFarmerStmt.get();

      if (testFarmer) {
        const farmerToken = testFarmer.user_token;
        const insertCowStmt = db.prepare('INSERT INTO dbt4 (cow_name, cow_nickname, collar_id, cow_token, user_token, farm_token, timestamp, state_fence, time_inside, time_outside, total_breach, alarm1_triggered, alarm2_triggered, alarm3_triggered) VALUES (?, NULL, ?, ?, ?, NULL, ?, \'inside\', 0, 0, 0, 0, 0, 0)');
        ...
        const updateStmt = db.prepare('UPDATE dbt1 SET total_cow = total_cow + 5 WHERE user_token = ?');
